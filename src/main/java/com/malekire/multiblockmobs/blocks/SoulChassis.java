...
public class SoulChassis extends Block{
    ...
    @Override
	 public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    	...
    	ParticleSpawner.spawnParticle(EobEnumParticleTypes.FLOWER, blockPos1.getX()+.5, blockPos1.getY()+1, blockPos1.getZ()+.5, 0.0D, -1.0D, 0.0D);
    	ParticleSpawner.spawnParticle(EobEnumParticleTypes.FLOWER, blockPos1.getX()+.5, blockPos1.getY()+2, blockPos1.getZ()+.5, 0.0D, -1.0D, 0.0D);
    	ParticleSpawner.spawnParticle(EobEnumParticleTypes.FLOWER, blockPos1.getX()+.5, blockPos1.getY()+1, blockPos1.getZ()+.3, 0.0D, -1.0D, 0.0D);
    ...
    private void trySpawnMultiblockMob(World worldIn, BlockPos pos)
    {
    	...
    	//System.out.println(Main.blockPatterns.size());
	    	...
	    	for(int i5 = 0; i5 < Main.blockPatternContainer.get(i6).size(); i5++)
	    	{
	    		...
	    		BlockPattern.PatternHelper blockpattern$patternhelper = theBlockPattern.match(worldIn, new BlockPos(pos.getX(), pos.getY(), pos.getZ()+i5));
	    	...
	    	else
	    	{
		    	...
		    	else
		    	{
		    		...
		    		for(int i5 = 0; i5 < Main.blockPatternContainer.get(i6).size(); i5++)
			    	{
			    		...
			    		BlockPattern.PatternHelper blockpattern$patternhelper = theBlockPattern.match(worldIn, new BlockPos(pos.getX()+i5, pos.getY(), pos.getZ()));
	    	...
	    	if(trueSuccess)
	    	{
	           ...
	           // worldIn.spawnEntity(entitysnowman);
	            	...
	            	/*
	            for (EntityPlayerMP entityplayermp : worldIn.getEntitiesWithinAABB(EntityPlayerMP.class, spawnedMob.getEntityBoundingBox().grow(5.0D)))
	            {
	                ...
	                CriteriaTriggers.SUMMONED_ENTITY.trigger(entityplayermp, spawnedMob);
	            }*/
        ...
        if (blockpattern$patternhelper != null)
        {
           ...
           // worldIn.spawnEntity(entitysnowman);
            if(ModChecker.isHarvesterLoaded)
            {
            	...
            	Entity harvester = ForgeRegistries.ENTITIES.getValue(new ResourceLocation("harvestersnight", "harvester")).newInstance(worldIn);
            	harvester.setLocationAndAngles((double)blockpos1.getX() + 0.5D, (double)blockpos1.getY() + 0.05D, (double)blockpos1.getZ() + 0.5D, 0.0F, 0.0F);
            	worldIn.spawnEntity(harvester);
            	//worldIn.playSound((double)blockpos1.getX(), (double)blockpos1.getY(), (double)blockpos1.getZ(), Main.harvesterLaugh, SoundCategory.AMBIENT, (float)10, (float)1, false);
            	harvester.playSound(Main.harvesterLaugh, 10, 1);
            for (EntityPlayerMP entityplayermp : worldIn.getEntitiesWithinAABB(EntityPlayerMP.class, harvester.getEntityBoundingBox().grow(5.0D)))
            {
            	entityplayermp.playSound(Main.harvesterLaugh, (float)100.0, (float)1.0);
            	entityplayermp.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, 10, 10, false, false));
    ...
    {
        return true;
        		//worldIn.getBlockState(pos).getBlock().isReplaceable(worldIn, pos) && worldIn.isSideSolid(pos.down(), EnumFacing.UP);
    }
    ...
    protected BlockPattern getSnowmanPattern()
    {
        ...
        {
        	Block block2 = ForgeRegistries.BLOCKS.getValue(new ResourceLocation("minecraft", "hay_block"));
            this.snowmanPattern = FactoryBlockPattern.start().aisle("^", "#", "#").where('^', BlockWorldState.hasState(IS_PUMPKIN)).where('#', Main.getBlock("minecraft", "hay_block")).build();
        }
    ...
    protected BlockPattern getGolemBasePattern()
    {
        ...
        {
            this.golemBasePattern = FactoryBlockPattern.start().aisle("~ ~", "###", "~#~").where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.WOOL))).where('~', BlockWorldState.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
        }
    ...
    protected BlockPattern getGolemPattern()
    {
        ...
        {
            this.golemPattern = FactoryBlockPattern.start().aisle("~^~", "###", "~#~").where('^', BlockWorldState.hasState(IS_PUMPKIN)).where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.WOOL))).where('~', BlockWorldState.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
        }
